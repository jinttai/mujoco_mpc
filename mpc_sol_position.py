...
class MPCParams:
    dt: float = 1
    N: int = 10
    Q_elem: np.array = np.array([10,10,10,1,1,1,1,1,1])
    Q: ca.DM = ca.DM(np.diag(Q_elem))
    R: ca.DM = ca.DM(np.eye(6))
    Qf: ca.DM = ca.DM(np.diag(Q_elem))

    u_min: ca.DM = ca.DM(-np.ones(6))
    u_max: ca.DM = ca.DM(np.ones(6))
...
class MPCSolver:
    def __init__(self, dt=1, N=10):
        ...
        # x = [r0, quat0, qm, u0, um]
        # quat0 = [q1, q2, q3, q0]
        self.x = ca.MX.sym('x', 25) # quat0(3:7), r0(0:3), qm(7:13), u0(13:19), um(19:25) 
        self.u = ca.MX.sym('u', 6)
        x_dot = dyn.dynamics(self.x, self.u)
        self.f = ca.Function('f', [self.x, self.u], [x_dot], {'jit': False, 'jit_options': {'compiler': 'gcc'}})
        y = dyn.output_position(self.x)
        self.h = ca.Function('h', [self.x], [y], {'jit': False, 'jit_options': {'compiler': 'gcc'}})
        ...
        self.Y_ref = ca.MX.sym('Y_ref', 9) # r_ee, t_ee
        ...
        opts = {'ipopt.print_level': 0, 'ipopt.max_iter': 1000, 'ipopt.tol': 1e-8, 
                'ipopt.acceptable_obj_change_tol': 1e-6, 'jit': False, 'jit_options': {'compiler': 'gcc','verbose': True}}
